...
automatically analyze build logs has thus far not been systematically overviewed as a research topic, bringing us to the first
research question:
...
To answer this question, we start our investigation---pictured in \Cref{fig:overview}---with an
...
% TODO fill-in X
only X independent implementations are available, one of which stems from the second author of this paper. Out of these implementations, all are based on either a custom parser or regular expressions.
%Using 1) a set of
...
As a consequence, we took it upon ourselves (step 2) to provide implementations of the three most promising automatic build log analysis techniques we observed in the literature survey:
a technique 1) able to automatically synthesize regular expressions, called \emph{Program Synthesis by Example (PBE)}, 2) based on natural language processing, doing \emph{Common Text Similarity (CTS)}  and 3) inspired by information-retrieval employing \emph{Keyword Search (KWS)}.
...
To answer this question, in the third step  of \Cref{fig:overview},
...
build tools and programming languages. Our results show that there is no technique that in general outperforms the others. However, 
% TODO 1-2 sentences about results, inclduing some accuracy measures

Finally, due to this ambiguity, we develop a set of guidelines (\Cref{fig:overview}, step 4)
...
\item a detailed research map to inspire future contributions to the field.
...
% TODO (resolved): All of these are passive
% TODO (resolved) Swap RQ 1.2 and RQ 1.1 (logical order?)
...
\includegraphics[width=0.55\columnwidth]{img/lit-sur/techniques-no-guidelines-cropped_3.pdf}
\\
% TODO: We need to mention in the text describing the Table what we mean by ``Analysis''
...
% TODO CARO
% start by defining in more detail what a chunk retrieval technique is
% or: characteristics that distinguishe them / to specify them further
% (To Moritz: we have short definitions before, the first time we use
% 'chunk', this is far to detailed to be earlier than here)
% Following that we present each of the techniques in detail.
...
% TODO CARO this is not enough for Table 3.
...
% TODO CARO move to end of characteristics section and say that it is
% summary of the techniques along the aspects introducted
% can lead reading / understanding of the future sections

% TODO QUESTION Somehow, we have to find a better place for this.
%We have been
% using the term 'chunk retrieval' beforehand already -- this
% definition now comes too late.
...
% TODO Caro: Can you given an example of such an error message?
...
% TODO: Ok, but what is the point of matching on an empty string
(if I understand what you mean)? you would not normally define such
an extraction?
% If this distinction is important it needs to be made more clear
...
% than RLR?
% TODO QUESTION: should I add those here? as references to the disc?
% or in the discussion? (you also already added some)
% In general: I think we talk a lot about RLR and it gets wordy.
...
% TODO QUESTION: Caro show whole structure
% -> introduction to crts, investigated techniques, ???
...
%of results?
% motivation why only one test example.
...
%equations.
This concludes one run within our evaluation.
...
% TODO QUESTION: yes, though then we have more redundant text
% that repeats everywhere and is already in the captions
% -> should we add it for the first time we include a type of figure?
...
% all of these techniques beat RLR handsdown -> discussion!
% TODO QUESTOIN: well they don't in this picture (PBE, multiple sc)
% maybe we can say that one figure earlier?
...
% TODO CARO Make intro less generic?
...
% also reference to the parts of the figure during the explanation
...
% TODO we can't just have a paragraph in a scientific paper start with Caveat, I think ;-) Also, we should say why we THINK this is not the case. Otherwise we weaken our work unduly
\textbf{Caveat!}
...
% TODO QUESTION: which one do you mean here?
...
% TODO CARO Do reference Figure 17 more, thoguh
