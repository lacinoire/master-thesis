...
of information in it which is linked to the error, is akin to
...
or investigate static analysis tools in CI
...
% TODO Moritz: 14 is still quite big, it's 'impl available' when we saw
% no overlap.
Decide whether you want to keep this in :)
only 14 independent implementations are available, one of which stems
...
Even though these three approaches---regular expressions, text similarity,
keyword search---
have different strengths and weaknesses, the literature
...
analysis.
We start with a differentiation to the related field of
system log analysis.
...
% it says resolved, but they still are?
...
and inspected each as closely as necessary to fill out the template
with confidence,
...
% remove following part of this paragraph?
% I think it is a nice anecdote but not super necessary yes.
...
% perhaps remove this paragraph?
% C: made it more specific, this is the only reference in the result
% which supports 'we need automatic because so many logs are analy'
...
article each (B).
From this, we follow: (a) there is a niche for custom parsers or even
...
% TODO CARO: make more clear that not matching is an error cause
...
% is explained :)
% TODO Slightly better, but I still have a problem with this -- what
% exactly is the vector
% space model here?
% At a minimum, the text should mention what the vector is and how the
% model is used
...
% FIXME introduce term example
% FIXME mention study figure
...
% TODO Yes, that sounds good.
% FIXME CARO You might even say Figure 12-15 show two
...
% TODO we can't just have a paragraph in a scientific paper start with
