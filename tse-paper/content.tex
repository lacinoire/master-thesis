...
an example of real-world log chunks and how the techniques can
retrieve them from build logs.
...
corresponding build failed, the information most often targeted by
other articles which analyze build logs.
...
CTS will rank the lines from the analyzed build log which are most
similar to the chunks in the training examples,
marked blue in \Cref{lst:chunk-example}.
...
The example from \Cref{lst:chunk-example-3} could be found with
``FAIL''.
If KWS would be trained with all three examples and the afformentioned
