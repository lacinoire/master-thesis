...
To answer this question, we start the investigation in this paper---pictured in
...
In this section, we report on the state-of-research in the emerging field of build log analysis by means of a systematic mapping study.
...
We know from other research lines that regular expressions, the second most popular way to analyze log files (\Cref{tab:litsur:techniques}), are tedious to
...
\Cref{fig:litsur:log_producer} shows that there are only a handful of build tools whose build logs more than one study analyzed, mainly in the Java ecosystem (area \circlenum{A}). Many works target a unique build tool that did not appear in any other study (area \circlenum{B}). 
