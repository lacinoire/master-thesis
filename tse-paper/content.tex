...
(\Cref{fig:overview}), we choose three promising
...
In the third step (\Cref{fig:overview}),
...
We develop a set of guidelines (step 4, \Cref{fig:overview})
on how to choose a
...
% TODO: is the future work good enough to put in contributions?
...
% TODO Swap RQ 1.2 and RQ 1.1 (logical order?)
\begin{itemize}[leftmargin=2cm]
  \item[\textbf{RQ1.1:}] Which information in build logs is of interest?
  \item[\textbf{RQ1.2:}] Which extraction techniques exist?
  \item[\textbf{RQ1.3:}] Which kinds of build logs are subject to analysis?
\end{itemize}
\end{simplebox}
...
% TODO: Ok, but what is the point of matching on an empty string (if I understand what you mean)? you would not normally define such an extraction?
...
{\textbf{RQ2:} How do build log analysis techniques compare against each other?}
...
did not improve the chunk retrieval. In fact, unless they
...
33.75\% two, 13.75\% more than two).  This stems from the fact that we
...
their decision on how to analyze build logs. To this end, we first
conducted a systematic mapping study to map out the available build
log analysis techniques.  We then
implemented three promising chunk retrieval techniques and compared
their performance on the \emph{LogChunks} data set.  Our results show
...
technique.  Secondary factors are the desired confidence in recall and
