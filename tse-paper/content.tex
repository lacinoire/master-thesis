...
% TODO Caro: Not 100% sure about the 6. I had a set of rlogs before
% that, but then I think you only always retrieve 1 build
% log. However, I also looked into the LogChunks data set and saw more
% than 6 logs per project sometimes ...

% TODO: In addition, if there
% are more than 6 logs per project, aren't we missing another
% indirection in the algorithm?


\begin{figure*}[tb]
\begin{algorithmic}[1]
\ForEach{technique $\in\{PBE,CTS,KWS,RLR\}$}
 \ForEach{project  $\in$ \emph{LogChunks}} \Comment{$|\emph{LogChunks}|$=80}
  \For{$i\gets 1, 5$} 
   \State $logs \gets getBuildLogsByTime(project, i)$  \Comment{Logs returned ordered by time of creation.}
   \State $rlog \gets  getBuildLogsByTime(project, 6)$
   \State $n \gets |structuralCategories(logs)|$
   \State
   \State $train(technique, logs)$
       \State $retrievedLines \gets run(technique, rlog)$
       \State $metrics \gets calculateMetrics(retrievedLines, desiredLines(rlog))$
       \State
...
\end{algorithmic}
	...
	\caption{Study procedure of the empirical chunk retrieval comparison study.}
	\label{fig:study}
\end{figure*}

In the following, we explain our study design
and introduce the metrics that
we measure to answer RQ2. \Cref{fig:study} depicts the individual steps of our study as an algorithmic procedure.

For each chunk retrieval technique and each project, we create training example sets consisting of 1, 2, \dots 5 examples (lines \circlenum{1}--\circlenum{3}).
We vary the training set size to measure how many examples we need to
confidently configure a technique, adressing \textbf{RQ2.1}.
...
a more recent log, just as one would apply these techniques in the real world.

In line \circlenum{6}, we obtain how many different structural categories are present in the trained logs and store this to be able to answer \textbf{RQ2.2}.
...
on the
training set, producing the retrieval output \circlenum{9}.

Based on this, we calculate the following metrics in \emph{calculateMetrics}, addressing \textbf{RQ2.3}:
...
\noindent
Finally, we aggregate the results to be able to reason per technique \circlenum{15}, completing our study design to answer \textbf{RQ2} overall.
...
\begin{itemize}[leftmargin=0.4cm]
  ...
  enumerating possible regular expressions~\cite{le2014flashextract:}.
...
Using a
  wordnet~\cite{pedersen2004wordnet} or other NLP techniques to
