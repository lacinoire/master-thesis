...
\begin{figure}[tb]
	...
	\caption{Literature selection (following Petersen et al.~\cite{petersen2015guidelines}).}
...
In sum, we classified 61 works in detail.
...
and inspected each works as closely as necessary to fill out the
template
with confidence,
starting from occurrences of ``log'' or ``build'' within the works's
text.
...
all works that used build logs provided by a company.
...
developers.
For 8\% of the works the retrieved information was the input
...
and several other works studied.
...
in the literature, with 61 works making use of it.
...
\begin{figure}[tbp]
		...
		\caption{High-level results of chunk retrieval with Program Synthesis by Example (PBE).}
...
[0K$ test/sass-compile-tester.sh
  ...
  \end{lstlisting}
	\caption{Actual chunk retrieval output ($RetrievedLines$ in yellow).}
...
\begin{figure}[!t]
		...
		\caption{Success of chunk retrievals across all techniques.}
