...
to automatically analyze build logs (step \circlenum{2}) by retrieving a substring (chunk)
from the build log.  Our
implementations are based on the most widely used approaches
from the literature survey and as such, are meant to serve as
the ``smallest common denominator,'' with as little tuning and optimization as possible to stand protoypical for the ideas they represent:
\begin{itemize}
...
\Cref{tab:techniques} summarizes the techniques, which we describe here in detail.
...
not emerge from the literature survey. Moreover, they typically
require lots of training and tuning. However, our premise was to ease
the creation of build log analysis for developers. First having to
label thousands of logs on their project before they can start to
analyze build logs is therfore infeasible. Similarly, as a manual
effort, the \emph{LogChunks} data set is not big enough to support the
training of such supervised techniques. Finding a way to train them in
...
analysis techniques, such as for example deep learning-based techniques (discussed in \Cref{sec:choice}) or manually-written programs such as the parser Tomassi et al.
created for
