...
% Moritz: I have added a link to the disqus site of TT
% (where this originates from and would leave it in the
% current state)? Ok with you?
...
% TODO Caro: check article vs work occurrences, balance use
...
\Cref{fig:failure-reason-PBE} depicts the year of appearance of these works. Although we applied no filtering by year in our initial search queries (see \Cref{sec:litsel}), the earliest work in our literature corpus stems only from 2001. There are various scattered works in the years between then and 2016, when the field of build log analysis started to grow rapidly.
...
A large fraction of the articles analyze build logs
...
% top to bottom. For now, it seems ok
...
% Would recommend to delete -- it simply follows later
