...
We did a second filter phase by downloading the surviving papers,
...
sampling for related work referenced in the paragraphs that mention
the technique. This added two papers (five before duplicate
elimination and second-pass filtering) to our literature set, both of
which stem from the same first author, were borderline-includes, and
had called build logs from the Linux kernel ``build traces.''
