...
% TODO Caro: Not 100% sure about the 6 in the
% algorithm below.
% I had a set of rlogs before
% that, but then I think you only always retrieve 1 build
% log.
% However, I also looked into the LogChunks data set and saw more
...
\ForEach{technique $\in\{PBE,CTS,KWS,RLR\}$}
 ...
 \Comment{$|\emph{LogChunks}|$=80}
  \For{$i\gets 1, 5$}
   \State $logs \gets getBuildLogsByTime(project, i)$  \Comment{Logs
...
\end{algorithmic}
	...
	\caption{Study procedure of the empirical chunk retrieval
	comparison study.}
	\label{fig:study}
\end{figure*}
