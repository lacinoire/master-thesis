...
% TODO Moritz: should we say 'our' experience? or 'one authors'?
% the latter might be too cumbersome
logs from different build tools, and is, as the experience of
...
plugin for Jenkins~\cite{jenkins2020junit-plugin}.
...
the plugin was installed cannot be analyzed.
...
build logs---provides these examples and keywords, which can simply be
...
% TODO Caro: unclear.
or do you mean: same number == all searched for ?
...
% find better one
\section{Empirical Comparison Study Design}
\label{sec:study}
...
\subsection{Study Method}
 ...
 gives an overview over the steps within .
In the following, we explain each of the steps of our study
(see \Cref{fig:study})
and introduce the metrics
...
% TODO Caro: check: where are the bean plots?
