...
Then, we motivate and present the choice of three such techqniues that
we implemented for this study in detail.
...
% TODO this should go under characteristics and not be in an introduction
to the section ...
...
The strategies that underlie them were inspired
...
generalize on the wide varitey of build logs we are interested in.
The
...
This
mimics regular expressions, the second most frequently occurring whose
while simplifying
their development.
...
The reason for it is that the relevant, failing parts of build logs
often looks similar to each other, even if the exact failure reason
might differ.
