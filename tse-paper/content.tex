...
build logs valuable data source, but semi-structured and verbose,
therefore need (automated) processing
...
We conduct an empirical study on the \emph{LogChunks} \todo{cite} data set to
...
\begin{itemize}
  ...
  Based on 
  \item \textbf{a common text similarity approach (CTS)}
  \item \textbf{keyword search (KWS), an ad-hoc method for finding fitting passages by searching the whole text for the occurrence of a specific trigger word}
\end{itemize}
...
% TODO: explain structural category based on running example from above?
...
package \todo{cite/add replication package}.
