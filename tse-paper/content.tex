...
% Moritz: I think best to decided once we read the paper
% top to bottom.
%For now, it seems ok
% Needs to go Threats or intro
% TODO Caro: I moved it to the intro
...
% systematically twice -- avoid this
...
% TODO: isn't this discussion already?
% Moritz: Not sure what is meant by that?
...
% TODO The numbers here might look a bit strange
% --- we say five but really it is six works we reference.
% I assume this is because of only including the oldest ref?
% Still, I think we need to fix the inconsistency.
...
% TODO	include the several others as references?
...
the almost equally much used categorization: one can deduce a categorization from chunks, but not vice versa.
...
% TODO This introduction is weird
In this section, we formally define the concept of chunk
retrievals from build logs. Then, we motivate and present the choice of three such techqniues that we implemented for this study in detail.
...
% TODO this should go under characteristics and not be in an introduction to the section ...
...
% TODO (resolved) Never define a concept with the
% words used in the term
% Use a thesaurus to find synonyms

With the term ``chunk retrieval technique,'' we refer to a way of automatically extracting strings that appear literally in build
...
Therefore, they
cover the great majority of the types of log analysis we saw in \Cref{sec:lit-sur:discussion}.
...
retrieval techniques. The strategies that underlie them were inspired
based on techniques we found in the literature survey and other
research fields. 
In \Cref{tab:techniques}, we summarize and distinguish the techniques
we present along the characteristics described above.

Techniques based on custom parsers (\#1
in \Cref{tab:litsur:techniques}) are inadquate for our comparison,
since their creation is manual and such a technique would not
generalize on the wide varitey of build logs we are interested in. The
closest we could come to the second entry, regular expression, is a
technique to automatically derive them: In particular, we implement a
technique based on
\emph{Program Synthesis by Example (PBE)}. This 
mimics regular expressions, the second most frequently occurring whose while simplifying
their development.

To work effectively with build logs, many humans anecdotally search
(``grep'') the logs for key phrases such as ``fail.'' Papers often
resorted to such a strategy when doing manual analysis (\#2
in \Cref{tab:litsur:techniques}), describing it with the word
``scan.'' Imitating this ad hoc approach of searching for specific
strings, we automatize it as \emph{Keyword Search (KWS)}.

Finally, from the fields of information retrieval and natural language
processing, we leverage a
\emph{Common Text Similarity (CTS)} approach. The reason for it is that the relevant, failing parts of build logs often looks similar to each other, even if the exact failure reason might differ. 
...
the real-world build logs in \Cref{lst:logexamples}.
