...
%automatically
% ^ we survey manual as well, though leaving it out includes too much
systematically
analyze build logs has thus far not been systematically
overviewed as a research topic, bringing us to the first
research question:
% TODO Discuss: is -research fine? or state-of-the-art more common and
% still not missused (because industry m)
...
% TODO Discuss/Moritz: should we say 'our' experience? or
% 'one authors'?
% the latter might be too cumbersome
logs from different build tools, and is prone to errors,
as the experience of
TravisTorrent has shown~\cite{beller2017travistorrent}.
...
% turned into an itemize again cause the 'sentence' around it was
% rather broken and item-ish anyways
...
by retrieving a substring (chunk) that describes a
...
% called \emph{Program Synthesis by Example (PBE)}, a technique
% 2) based on information
% retrieval, using \emph{Common Text Similarity (CTS)}	and 3)
% inspired by the manual ad hoc search for key parts in the log, a
% technique employing \emph{Keyword Search (KWS)}.
...
field.
% (resolved) only point with no adjective
...
\begin{itemize}
  ...
  what this information is used for (\textbf{RQ1.1}),
  \item which techniques they to process the build logs,
  in how much detail the techniques are described,
	and whether their implemented tools are published
  (\textbf{RQ1.2}),
  \item what kind of build logs the researchers analyze
...
We then present the results of the analysis phase, along the
RQs 1.1 through 1.3.
...
In this section, we describe what sort of information the articles in
our survey usually retrieved from build logs.
These results are depicted in \Cref{fig:litsur_r}.
...
Of the 61 articles we inspected, 43 mentioned how they analyzed
build logs.
...
% TODO Caro: I think the axes labels are also unnecessary for this graph.
...
% ----- information RQ 1.1 -----
In regards to the information which was targeted, \textbf{most build log
...
% not sure if we can state that (no data on this, more 'impression')
...
% have a different meaning (we want build tool -> study count)
...
In most cases, \textbf{researchers analyze build logs from open
source projects}, obtained through Travis CI.
In addition, we saw several articles that targeted the logs of only
...
% TODO Moritz: is this okay (here)?
% maybe it's better to add it to Threats to Validity with the heading:
% \texbf{Modifying the Build Process.}
...
In addition to that, when using such a plugin, one would have to know
beforehand which information should be reported from a build, whereas
build log analysis enables us to analyze the wide range of information
...
detail.
At the end of this section we illustrate how the techniques
...
\subsection{A New Generation of Build Log Analysis Techniques}
In summary, the results of the literature survey show the need for
build log
...
build logs---provides these examples and keywords, which are easy to
adapt whenever the analysis needs to be maintained.
...
we create a data set with logs from failed builds on Travis
CI---the most-used source of build logs within our survey.
For each log we marked the corresponding
...
build tools which enables us to measure the generic
...
%the paragraph about why chunk log techniques are more general
...
intent of the user through examples which they provide in an automatically
generated program.
...
% bothers you :)
% no match is a category later in the results section
...
Then, we merge these lists and use the most commonly occuring
...
% TODO Moritz: reference these in text properly
...
=== RUN   TestSeparator
  ...
  \vspace{-\baselineskip}
  % TODO Caro: Adapt frame= option to close frame properly
  \begin{lstlisting}[breaklines=true,frame=blr]
...
% TODO Moritz: is this fitting here?
% or too much 'foreshadowing' too far down in the hierarchy
...
In the following we will introduce the data set we created for this
...
For each file, we manually marked
the chunk that describes why the build
failed---the information targeted by most of the articles from our
literature survey.
...
was 47\%.
\Cref{lst:pbe-partially-successful} shows how an example of such
...
% TODO Caro: bring more across why it is so important that this field
% and the research there continues
Finally, we gave an
