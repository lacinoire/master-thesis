...
Classification is the more abstract representation, chunks
contain more information and could be turned into a classification.
...
classified the types of compile errors encountered at
Google.
...
{Parser} &
\citep{vassallo2018un-break,zhang2016android,seo2014programmers,hassan2019tackling,hassan2017automatic,chromy2007integration,mesbah2019deepdelta,wen2018blimp,kwon2018prioritizing,adams2007design,rahman2018impact,brandyberry2006continuous,tomassi2019bugswarm,ren2018automated,vassallo2019automated,cavalcanti2019impact,sippola2013qt,felipe2012towards,shi2018evaluating,urli2018design,selberg2012use}
 &
\includegraphics[width=0.35\columnwidth]{img/lit-sur/techniques-no-guidelines-cropped_21.pdf}
\\

{Regular expression} &
\citep{beller2017oops,hassan2017change,macho2018automatically,vassallo2017a-tale,lou2019history,hassan2017automatic,rott2019empirische,zampetti2019study,zhao2018comparing,rausch2017empirical,ghaleb2019studying,zampetti2017open,zhang2019large,kavaler2019tool,morris2010experience}
 &
...
the almost equally much used classification: one can deduce a
classification from chunks, but not vice versa.
...
The chunk in \Cref{lst:chunk-example-3} is very
different.
For example, it does not have the two extraneous newlines
after the chunk.
\caro{introduce sturctural categories here?}
...
CTS will rank the lines from an analyzed build log whose vocabulary is
most similar to the chunks in the training examples.
It will then return the two lines ranked highest, as the chunks in
the training examples have on average 2.0 lines.
In this example,
CTS, trained on \Cref{lst:chunk-example},
will have difficulty extracting the lines from
...
To configure KWS, the user has to provide search terms.
If log excerpts such as the ones from \Cref{lst:chunk-example}
are targeted, possible search terms would be
...
particular) inspired the strategies that underlie them and our central
requirement of enabling a user to configure the technique to their use
...
We conduct an empirical study to compare how well the three
configurable chunk retrieval techniques can be applied to analyze
build logs.
With this evaluation we want to give a future user guidance on which
technique to choose for their build log analysis use case.

To understand the amount of configuration necessary, we investigate
how many log excerpts a technique
should be trained with (\textbf{RQ2.1}),
how similar the structural representation of the targeted log chunks
has to be (\textbf{RQ2.2}).
To determine whether the output of the build log analysis is suited
to be analyzed automatically or by a human user, without the need to
refer back to the full build log, we measure how
reliable the quality of the
...
study, describe the study design and our results.
We discuss the answers to the RQs separately for each technique
and compare them to give recommendations on which technique
a future user should configure to analyze their build logs.
...
\begin{itemize}[leftmargin=0.4cm] \itemsep1em
  ...
  \todo{motivate this distinction: partially means not suited for
  automatic processing? unsuccessful for have to go back to log}
...
successful or unsuccessful in retrieving the complete
chunk defined in the test example.
CTS and KWS
...
we discuss how accurate the three different techniques retrieve
information from build logs and infer how their
output can be used further.
Further, we give recommendations for
which types of log chunks and how many training examples
each technique is suited.
