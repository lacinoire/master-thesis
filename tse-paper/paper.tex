...
\subtitle{A~Systematic Survey and Comparative Study of Chunk~Retrieval~Techniques}
...
In a broad literature mapping survey, we first survey and categorize the existing methods to
...
techniques to analyze build logsx, namely program synthesis
...
build log analysis.
\keywords{Build Log Analysis \and Chunk Retrieval \and Continuous Integration}
\end{abstract}
...
build process
 such as the JUnit
plugin for Jenkins~\citep{jenkins2020junit-plugin} or
Deflaker by Bell et al.~\citep{bell2018deflaker}.
...
TravisTorrent has
shown~\citep{beller2017travistorrent,travistorrentquestions}.
...
field.
\item a documented replication package~\citep{brandt2020chunk-replication}.
\end{itemize}
...
al.~\citep{petersen2015guidelines}, we started the mapping survey with a
pilot:
...
Maven~\citep{maven2019website},
16\% analyzed Gradle~\citep{gradle2020website},
and 10\% analyzed Ant build logs~\citep{ant2020website}.
...
for build logs.
The \emph{PROSE library}~\citep{prose2019webpage} forms the basis for
our implementation.
...
% consistent~\citep{mitchell1982generalization} with a set of in-
% and output examples.
...
library~\citep{prose2019webpage} and CTS, KWS, and RLR using
R and the {\tt text2vec} library~\citep{text2vec2019webpage}.
...
analyzer~\citep{jenkins2020failure-analyzer}, where a user can
...
{\tt text2vec}~\citep{text2vec2019webpage}
...
plugin~\citep{jenkins2020failure-analyzer} or automatically inferred
test results in Azure~\citep{azure2020inferred}.
Such a study might
