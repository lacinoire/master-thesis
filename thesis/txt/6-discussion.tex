...
After discussing three chunk retrieval techniques separately we now want to unify our results into one recommendation scheme.
...
We also recommend CTS when the representations are from one structural category, the user does not require a high confidence in the precision or recall of the outcome and the user would rather have output with low recall instead of no output at all.
