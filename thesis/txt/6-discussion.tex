...
For example, the need for examples from a single structural category stems from the fact that FlashExtract cannot learn regular expression programs with (too many?) top level OR operators. \mention{CITATION NEEDED}
...
Especially the results for CTS are influenced by the fact that we only trained on one to five examples.
Information retrieval techniques commonly learn on larger data sets. \mention{CITATION NEEDED}
