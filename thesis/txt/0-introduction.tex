...
how many training examples a technique needs to perform best (\textbf{RQ2.1}), how structurally diverse the examples can be (\textbf{RQ2.2}) and the accuracy of the retrieved output (\textbf{RQ2.3}).
...
\begin{itemize}
  ...
  A small increase in the number of training examples has no visible influence.
  Fewer structural categories improve the retrieval accuracy.
  \item KWS has the highest recall of all techniques combined with much lower precision.
...
Chapter~\ref{sec:rw} also presents existing work on information extraction and retrieval techniques and program synthesis from examples.
...
Following you find selected graphs from the study results section. RLR stands for the baseline technique \emph{Random Line Retrieval}:
...
\begin{figure}[htbp]
		...
		\caption{Precision and Recall of all techniques compared when training examples are in \emph{multiple structural categories}}
